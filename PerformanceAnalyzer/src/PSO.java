...
public class PSO {
	...
	public static void main(String args[]) {//pso(List<List> concreteList){
        double w         = 0.9, 
               c1        = 2.05,  c2        = 2.05, 
               r1        = 0.0,   r2        = 0.0,
               xMin      = -5.12, xMax      = 5.12,
        ...
        for(int p=0;p<Np;p++)
        {	for(int i=0; i<Nd; i++)
        	System.out.print(" "+R[p][i]);
    	System.out.println();
        }
    
        //for(int p=0; p<Np; p++){
           // M= Fitness.fitness(concreteList,Np,R);   un
         //   M[p] = -M[p];
       // }
        ...
        for(int j=0; j<Nt; j++){ 
            for(int p=0; p<Np;p++){         
                for(int i=0; i<Nd; i++){    
                    R[p][i] = (int) (R[p][i] + (int)V[p][i]);

                    if(R[p][i] > xMax)          { R[p][i] =(int) xMax;}
                    else if(R[p][i] < xMin)     { R[p][i] =(int) xMin;}
                }           
            }   
